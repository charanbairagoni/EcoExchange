(function () {
  ...
  function render() { var current = route(); if (current.indexOf('buy/') === 0) return buyDetail(current.split('/')[1]); if (current.indexOf('seller/') === 0) return sellerProfilePage(current.split('/')[1]); if (current === 'home') return home(); if (current === 'about') return about(); if (current === 'register') return authPage('register'); if (current === 'login') return authPage('login'); if (current === 'profile') return profile(); if (current === 'dashboard') return dashboard(); if (current === 'sell') return sell(); if (current === 'buy') return buy(); if (current === 'admin' || current.indexOf('admin/') === 0) return admin(); return home(); }
